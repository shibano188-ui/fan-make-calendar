...
:root {
  ...
  --bg-primary:   #0e0e10;      /* 旧 #1a1a1a → 純黒(#000)より柔らかい折衷案【決定済み】 */
  ...
  /* アクセント: グレー → iOS systemBlue (dark)【決定済み】 */
...
- 右アクションの3連ボタン（地図・パレット・⋮）は**3つとも維持する【決定済み・統合しない】**。
  スタイルのみ変更: `bg-bg-secondary` の角丸四角 → **背景なしのアイコンボタン**
  （label-secondary、視覚アイコン18px、44pxタップ領域、`gap-0`で詰めて視覚ノイズを削減）。
  地域フィルター有効時のアクセントドットは維持。
...
## 6. ユーザー決定事項（2026-06-10 確定）

1. **アクセント色**: iOS systemBlue **#0A84FF** に決定。
   （ユーザーは Customize でいつでも変更可能。これはデフォルト値）
2. **背景色**: 純黒ではなく **#0e0e10** に決定（柔らかさ重視の折衷案）。
3. **ヘッダー3連ボタン**: **統合しない**。3つとも維持し、スタイルのみ刷新（3-3参照）。
