...
- [ ] ライト/ダーク/システム/コミュニティ全テーマで、全画面に読めない文字・見えない線がない
- [ ] ハードコード白の grep 結果が PhoneFrame の枠＋意図的な白のみ
- [ ] `window.confirm` / `alert` が 0 件
- [ ] 初回起動でオンボーディング → 作品選択まで迷いなく到達できる
- [ ] すべてのアイコンボタンのタップ領域が 44px 以上
- [ ] ネイティブビルドでステータスバーがテーマに追従する
- [ ] `npx tsc --noEmit` / `npm run build` が通り、Vercel Preview で両テーマのスクリーンショットを取得済み
