...
import { ChevronDown, ChevronUp, X, Plus, Sparkles, Link2, Camera, Loader2 } from 'lucide-react';
...
type SmartInputTab = 'url' | 'image';

function SmartInputPanel({ onApply }: { onApply: (parsed: ParsedEvent) => void }) {
  const [open, setOpen]       = useState(false);
  const [tab, setTab]         = useState<SmartInputTab>('url');
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      parseAndApply({ imageBase64: base64, mimeType: file.type });
    };
      ...
      {open && (
        ...
        >
          ...
          <div className="flex gap-1 mt-3">
            {(['url', 'image'] as SmartInputTab[]).map(t => (
              <button
                key={t}
                onClick={() => setTab(t)}
